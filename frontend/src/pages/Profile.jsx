...
export default function Profile() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await api.post('/me?_method=PUT', data, {
                            ...
                            <div className="px-8 pb-10 -mt-16 text-center">
                                <div className="relative inline-block mb-6">
                                    <div className="w-32 h-32 rounded-full bg-white p-1 border border-gray-100 shadow-2xl mx-auto overflow-hidden">
                                        {preview ? (
                                            <img src={preview} alt={user?.name} className="w-full h-full object-cover rounded-full" />
                                        ) : (
                                    ...
                                    <label htmlFor="user-photo" className="absolute bottom-2 right-2 w-8 h-8 rounded-full bg-indigo-600 border-4 border-white flex items-center justify-center text-white shadow-lg cursor-pointer hover:bg-indigo-700 transition-all">
                                        <Save size={14} />
                                    </label>
                                    <input type="file" id="user-photo" className="hidden" accept="image/*" onChange={handleFileChange} />
                                </div>
                    ...
                    <div className="lg:col-span-8">
                        <form onSubmit={handleSubmit} className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-gray-100 overflow-hidden">
                            <div className="p-8 sm:p-14 space-y-16">
                                ...
                                {/* Status Alerts */}
                                {status.message && (
                                    <div className={`flex items-center gap-4 p-6 rounded-2xl border-2 animate-in fade-in slide-in-from-top-2 duration-300 ${status.type === 'success' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-rose-50 border-rose-100 text-rose-700'
                                        }`}>
